...
//     // Step 6: Check if an entry in `tbl_info` already exists with the same `bid_bcdate_id` and `bid_sch_id`
//     const checkInfoEntryQuery = "SELECT * FROM tbl_info WHERE t_mem_id = ? AND sch_id = ?";
//     const existingInfoEntry = await query(checkInfoEntryQuery, [bid_mem_id, bid_sch_id]);

//     if (existingInfoEntry && existingInfoEntry.length > 0) {
...
//         UPDATE tbl_info 
//         SET status = ?, checkcount = ? 
//         WHERE t_mem_id = ? AND sch_id = ?`;
//       const updateValues = [
//         status,
//         count,
//         bid_mem_id,
//         bid_sch_id,
//       ];
//       await query(updateInfoQuery, updateValues);
//     } else {
...
//     const checkInfoEntryQuery1 = "SELECT status FROM tbl_info WHERE t_mem_id = ? AND sch_id = ?";
//     const existingInfoEntry1 = await query(checkInfoEntryQuery1, [bid_mem_id, bid_sch_id]);
...
//     return res.status(200).send({
//       status: true,
//       message: "Transaction entry inserted/updated successfully",
//     });
//   } catch (error) {
//     console.error(error);
//     return res.status(500).send({ status: false, message: "Internal Server Error" });
...
const trasactionentrymember3 = async (req, res) => {
  try {
    ...
    // Step 7: Check the status in `tbl_info` for further updates in `tbl_schememember`
    const checkInfoEntryQuery1 = "SELECT status FROM tbl_info WHERE t_mem_id = ? AND sch_id = ?";
    const [existingInfoEntry1] = await query(checkInfoEntryQuery1, [bid_mem_id, bid_sch_id]);

    if (existingInfoEntry1 && existingInfoEntry1.status === 1) {
      // Update `tbl_schememember` only if status is 1
      const updateStatusQuery = `
        UPDATE tbl_schememember 
        SET schmem_status = ? 
        WHERE schmem_sch_id = ? AND schmem_mem_id = ? AND schmem_status != 1 
