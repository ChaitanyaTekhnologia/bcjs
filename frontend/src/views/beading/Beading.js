...
import { checkMember, fetchbcdateandbcno, getDataId, updatebcstatus } from "../store/apiService";
...
const Beading = () => {
  ...
  // const defaultDate = bcdatabcno[0]?.bc_date
  //   ? formatDate(new Date(bcdatabcno[0].bc_date))
  //   : formatDate(currentDate); 

  console.log("hghf",bcdatabcno[0]?.bc_date)
  const [value, setValue] = useState({
    bid_name: "",
    bid_amount: "",
    bid_month: bcdatabcno[0]?.bc_date ? new Date(bcdatabcno[0].bc_date).toISOString().split("T")[0]  : "",    
     bid_sch_id: schemeId,
  });
  
console.log("gyug",value)
  ...
  console.log("Fetched data:", bcdatabcno);
  ...
  useEffect(() => {
    ...
    const month =  bcdatabcno[0]?.bc_date ? new Date(bcdatabcno[0].bc_date).toISOString().split("T")[0]  : "";
                ...
                <Row>
                  <Col>
                    <div>
                      ...
                      <strong>{bcdatabcno.length > 0 ? `BC-${bcdatabcno[0].bc_no}` : "Loading..."}</strong>
                      </h4>
